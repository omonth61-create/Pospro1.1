...
interface Props {
  onClose: () => void;
  onConfirm: (customerName: string, paymentMethod?: 'cash' | 'debt', paidAmount?: number) => void;
}
...
const CheckoutModal: React.FC<Props> = ({ onClose, onConfirm }) => {
  ...
  const handleFinish = async () => {
    ...
    if (name) {
      try {
        ...
        if (existing) {
          ...
          const now = Date.now();
          const newId = `cust_${customerPhone || crypto.randomUUID().slice(0, 8)}`;
    ...
    // ✅ 3. Teruskan Konfirmasi Transaksi POS ke Parent Page
    onConfirm(
      customerName, 
      paymentMethod, 
